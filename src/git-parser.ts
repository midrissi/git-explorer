...
export function explainGitObject(obj: GitObject): string[] {
  ...
  switch (obj.type) {
    case "blob":
      explanations.push(
        `This is a **blob** object — it stores the content of a single file.`
      );
    ...
    case "tree":
      explanations.push(
        `This is a **tree** object — it represents a directory listing.`
      );
      explanations.push(`Size: ${obj.size} bytes`);
      explanations.push(`Contains ${obj.entries.length} entr${obj.entries.length === 1 ? "y" : "ies"}:`);
